...
11. Add Filters for the files you want to translate. You can add filters for the files in the `translationsDirectory` specified in the `config.json` file.
   ...
   These are the configuration used;
   Source files: `/docs/build/**/*.qmd`. 
   Translated files path: `/docs/_translations/%two_letters_code%/**/%file_name%.%two_letters_code%.qmd`.

   Note that `/docs/build`  and `/docs/_translations/` are correspond with the values we set for `outputDirectory` and `translationsDirectory`  in the `config.json` file.

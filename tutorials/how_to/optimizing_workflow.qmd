...
```bash
npm run dev --source=/path/to/your/source/files 
```

To specify the tutorial files directory:
```bash
npm run dev --tutorial=/path/to/your/tutorial/files
```

To specify the supported languages:
```bash
npm run dev languages=en,fr,es
```

To include localized versions of the generated documentation:
```bash
npm run dev include_localized_versions languages=en,fr,es
```
